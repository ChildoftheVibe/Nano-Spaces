...
Phases are listed in order. All phases through Phase 15 are complete.
...
### Phase 15 — Production Readiness & Launch (complete)

**PWA (`public/`):**

- `manifest.json` — name "Nano Spaces", short_name "NanoSpaces", theme_color "#4F7EFA", start_url "/calendar", standalone display, keyboard shortcut to calendar
- `icon-192.png`, `icon-512.png`, `icon-badge.png` — brand-colour PNG icons generated by `scripts/generate-icons.mjs` (pure Node.js, no deps, zlib + CRC-32)
- `sw.js` (extended) — install-time shell caching (manifest, icons, /calendar), stale-while-revalidate for page assets, network-only passthrough for `/api/` and `/_next/`, improved push notification handler with vibrate + window focus

**Security headers (`next.config.js` + `middleware.ts`):**

- `next.config.js` — HSTS (`max-age=63072000; includeSubDomains; preload`), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Permissions-Policy: camera=(), microphone=(), geolocation=(), payment=()`, long-lived cache for `/_next/static/` and PWA assets, `poweredByHeader: false`
- `middleware.ts` — generates a per-request nonce (`crypto.randomUUID()` → base64), threads it via `x-nonce` request header so layout can read it, and sets full CSP on every response (including redirects): `script-src 'nonce-...' 'strict-dynamic'`, `style-src 'unsafe-inline'` (Tailwind requirement), allowlist for Supabase, Sentry, Cloudflare Turnstile, PayPal
- `app/layout.tsx` — reads nonce from `headers()`, adds manifest link, apple-touch-icon, PWA viewport meta, exposes nonce via inline `<script nonce>` for Next.js hydration

**Error boundaries:**

- `app/global-error.tsx` — catches root layout crashes; self-contained `<html>` shell, reports to Sentry, shows styled fallback without any component dependencies
- `app/error.tsx` — root segment error boundary; Sentry capture on mount, "Try again" + "Report this issue" buttons (calls `Sentry.showReportDialog()`)
- `app/(user)/error.tsx`, `app/(org-admin)/error.tsx`, `app/(super-admin)/error.tsx` — per-segment boundaries with "Back to calendar" navigation

**Health check (`app/api/health/route.ts`):**

- `GET /api/health` — pings DB (select from `tos_versions`), Storage (`listBuckets`), Redis (`ping`), email (env var check); returns `{ status: 'healthy'|'degraded', buildSha, version, timestamp, services }` with per-service latency; 200 or 503

**Onboarding checklist:**

- `app/api/org/onboarding-checklist/route.ts` — `GET`: checks 5 milestones (room added, availability rules set, team member invited, first booking made, email prefs configured); availability_rules joined via location_id (no org_id on that table)
- `components/features/onboarding/onboarding-checklist.tsx` — floating bottom-right card; progress bar; collapse/dismiss; 7-day localStorage dismissal per org; auto-hides if all complete; shown only to `org_admin` on the calendar page

**Shepherd.js tour (`components/features/tour/app-tour.tsx`):**

- Dynamically imported (keeps shepherd.js out of initial bundle)
- 4 steps: nav bar, calendar area, search, notifications bell — each targets a `data-tour="..."` attribute
- Skips steps whose target element isn't present
- Marks done in localStorage (`ns_tour_done`); loads Shepherd CSS from CDN
- `data-tour` attributes added to `app/(user)/layout.tsx`

**Empty states + skeletons:**

- `components/ui/empty-state.tsx` — `EmptyState` component with icon, title, description, optional CTA button or link
- `components/ui/skeleton.tsx` — `Skeleton`, `SkeletonCard`, `SkeletonRow`, `SkeletonTable`, `SkeletonStat` components (animate-pulse)
- `app/(org-admin)/users/page.tsx` — upgraded empty state with EmptyState component; skeleton table during load
- `app/(org-admin)/rooms/page.tsx` — skeleton card grid during load

**Dependabot (`.github/dependabot.yml`):**

- Weekly npm + GitHub Actions updates; minor/patch grouped into single PR; major Next.js/React pinned for manual review

**Dependency pinning:**

- All `^` and `~` prefixes removed from `package.json` dependencies and devDependencies

**Operational docs (`docs/`):**

- `secret-rotation.md` — rotation schedule and step-by-step procedure for every secret, including emergency rotation and storage rules
- `incident-response.md` — P0–P3 severity definitions, on-call checklist, 6-step response process, communication templates, escalation contacts, post-mortem template
- `disaster-recovery.md` — RTO < 4h / RPO < 24h targets, backup strategy, 5 restore scenarios (rollback, PITR, region outage, nuclear, Vercel loss), runbook commands, quarterly DR test checklist, data retention table

**Items requiring external action (not buildable in code):**

- Vercel production deploy + custom domain + SSL: done via Vercel dashboard once repo is pushed
- Preview deployment protection: Vercel dashboard → Settings → Deployment Protection
- Status page: set up via statuspage.io free tier or Betterstack
- PayPal live mode test: perform a real $0.01 subscription charge and refund
- Dependabot: automatically activates once `.github/dependabot.yml` is merged to main
- Sentry verification: trigger a test error and verify receipt with PII scrubbing
- securityheaders.com scan: run after domain is live with HTTPS

---

## What Remains to Build

All 15 phases are complete. **Nano Spaces is production-ready.** 🚀

External steps before go-live: Vercel deploy + custom domain, Supabase production project, all env vars in Vercel, status page, PayPal live mode test, securityheaders.com verification.

import type { NextRequest } from 'next/server'
import { withErrorHandling } from '@/lib/api-response/handler'
import { success } from '@/lib/api-response/helpers'
import { createSessionClient, createAdminClient } from '@/lib/supabase/server'
import { AuthError, NotFoundError, ValidationError } from '@/lib/errors/AppError'

const ALLOWED_SIGNATURES: Array<[number[], string]> = [
  [[0xff, 0xd8, 0xff], 'image/jpeg'],
  [[0x89, 0x50, 0x4e, 0x47], 'image/png'],
  [[0x52, 0x49, 0x46, 0x46], 'image/webp'],
]

function detectMimeType(bytes: Uint8Array): string | null {
  for (const [sig, mime] of ALLOWED_SIGNATURES) {
    // eslint-disable-next-line security/detect-object-injection
    if (sig.every((b, i) => bytes[i] === b)) return mime
  }
  return null
}

export const POST = withErrorHandling(
  async (req: NextRequest, ctx: { params?: { id?: string } }) => {
    const requestId = crypto.randomUUID()

    const sessionClient = await createSessionClient()
    const {
      data: { user },
      error,
    } = await sessionClient.auth.getUser()
    if (error || !user) throw new AuthError({ userMessage: 'Not authenticated.', requestId })

    const { data: profile } = await sessionClient
      .from('profiles')
      .select('role, org_id')
      .eq('id', user.id)
      .single()

    if (!profile || profile.role !== 'org_admin' || !profile.org_id) {
      throw new AuthError({ userMessage: 'Org admin access required.', requestId })
    }

    const roomId = ctx.params?.id
    if (!roomId) throw new ValidationError({ userMessage: 'Missing room id.', requestId })

    const admin = createAdminClient()
    const { data: room } = await admin
      .from('locations')
      .select('id, org_id, photo_url')
      .eq('id', roomId)
      .single()

    if (!room || room.org_id !== profile.org_id) {
      throw new NotFoundError({ userMessage: 'Room not found.', requestId })
    }

    const formData = await req.formData().catch(() => null)
    const file = formData?.get('photo')

    if (!(file instanceof File)) {
      throw new ValidationError({ userMessage: 'A photo file is required.', requestId })
    }

    if (file.size > 5 * 1024 * 1024) {
      throw new ValidationError({ userMessage: 'Photo must be 5MB or smaller.', requestId })
    }

    const buffer = await file.arrayBuffer()
    const bytes = new Uint8Array(buffer)
    const mimeType = detectMimeType(bytes)

    if (!mimeType) {
      throw new ValidationError({
        userMessage: 'Only JPEG, PNG, and WebP images are allowed.',
        requestId,
      })
    }

    const ext = mimeType === 'image/jpeg' ? 'jpg' : mimeType === 'image/png' ? 'png' : 'webp'
    const filename = `${crypto.randomUUID()}.${ext}`
    const storagePath = `${profile.org_id}/${roomId}/${filename}`

    // Remove old photo
    if (room.photo_url) {
      const oldPath = (room.photo_url as string).split('/room-photos/').at(1)
      if (oldPath) await admin.storage.from('room-photos').remove([oldPath])
    }

    const { error: uploadError } = await admin.storage
      .from('room-photos')
      .upload(storagePath, bytes, { contentType: mimeType, upsert: false })

    if (uploadError) throw new Error('Failed to upload photo.')

    const { data: urlData } = admin.storage.from('room-photos').getPublicUrl(storagePath)

    await admin.from('locations').update({ photo_url: urlData.publicUrl }).eq('id', roomId)

    return success({ photoUrl: urlData.publicUrl })
  },
)

export const DELETE = withErrorHandling(
  async (_req: NextRequest, ctx: { params?: { id?: string } }) => {
    const requestId = crypto.randomUUID()

    const sessionClient = await createSessionClient()
    const {
      data: { user },
      error,
    } = await sessionClient.auth.getUser()
    if (error || !user) throw new AuthError({ userMessage: 'Not authenticated.', requestId })

    const { data: profile } = await sessionClient
      .from('profiles')
      .select('role, org_id')
      .eq('id', user.id)
      .single()

    if (!profile || profile.role !== 'org_admin' || !profile.org_id) {
      throw new AuthError({ userMessage: 'Org admin access required.', requestId })
    }

    const roomId = ctx.params?.id
    if (!roomId) throw new ValidationError({ userMessage: 'Missing room id.', requestId })

    const admin = createAdminClient()
    const { data: room } = await admin
      .from('locations')
      .select('id, org_id, photo_url')
      .eq('id', roomId)
      .single()

    if (!room || room.org_id !== profile.org_id) {
      throw new NotFoundError({ userMessage: 'Room not found.', requestId })
    }

    if (room.photo_url) {
      const oldPath = (room.photo_url as string).split('/room-photos/').at(1)
      if (oldPath) await admin.storage.from('room-photos').remove([oldPath])
    }

    await admin.from('locations').update({ photo_url: null }).eq('id', roomId)

    return success({ deleted: true })
  },
)
